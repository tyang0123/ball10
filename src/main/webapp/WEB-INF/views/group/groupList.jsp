...
<div class="row" style="text-align: center; margin-top: 40px; margin-bottom:40px;position: relative;">
    ...
    <div class="col-2">
        <button style="width: 60px; position:absolute; right: 12vw;" type="button" class="button-timer-custom"
                id="createBtn">등록
        </button>
    </div>
</div>
        ...
        <div class="col-lg-12">
            <form id="listForm" action="/group/list" method="get">
                <select class="category" name="category">
                    ...
                    <option value="토익"
                            <c:out value="${cri.category eq '토익'?'selected':''}"/>>토익
                    </option>
                    <option value="취업"
                            <c:out value="${cri.category eq '취업'?'selected':''}"/>>취업
                    </option>
                    <option value="자격증"
                            <c:out value="${cri.category eq '자격증'?'selected':''}"/>>자격증
                    </option>
                    <option value="이직"
                            <c:out value="${cri.category eq '이직'?'selected':''}"/>>이직
                    </option>
                </select>
...
<div class="row" id="groupRow">  <!-- groupList -->
    <div style="background-color: #efefef; margin-top: 20px; padding-top:20px; padding-bottom: 80px;"
         class="center-block">
        <c:if test="${list!=null&&list.size()>0}"><c:forEach var="list" items="${list}">
            <div class="card user-card-group" style="cursor: pointer;" value="${list.group_is_secret}">
                ...
                <div class="card-body">
                    ...
                    <div>
                        <span class="group-list-title">목표시간 : </span><span
                            class="group-list-content">${list.group_target_hour} ${list.group_target_minute}</span><span
                            class="group-list-title"> 그룹인원 : </span><span
                            class="group-list-content">${list.group_join_person_number}/${list.group_person_count}명</span><span
                            class="group-list-title">  그룹장 : </span><span
                            class="group-list-content">${list.user_nickname_group_header}</span>
                    </div>
                    <div>
                        <span class="group-list-title">공부량 : </span><span class="group-list-content">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <fmt:parseDate var="timeparse" type="time" timeStyle="FULL"
                                                   value="${list.group_accumulated_avg_time}" pattern="HH:mm:ss"/>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <input type="text" id="inputPass" maxlength='20' class="form-control" aria-describedby="pwHelpInline"
                       placeholder="비밀번호를 입력하세요" style="border: black 1px solid;margin-top: 20px;"/>
            ...
            <div class="modal-footer" style="border-color:black;">
                ...
                <button style="width: 150px;" type="button" class="button-add-custom" onclick="reset()"
                        data-bs-dismiss="modal">취 소
                </button>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function () {
        $(".center-block").on('click', '.user-card-group', function (e) {
            ...
            if ($(this).attr('value') == 1) {
                ...
                $("input[id=inputPass]").keydown(function (key) {
